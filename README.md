...
смещена от центра на некоторое расстояние, из-за исходное
...
![Лабораторная работа 1](photo/LB1_MBR.gif)
...
конфигурационном файле `parameters.yaml`.
При `dynamic_update = True` условие выполняется на каждом кадре, 
...
состояния карты. С другой стороны, при высокой скорости движения может
возникать запаздывание, если алгоритм не успевает перестроить путь до
столкновения. В рамках лабораторной работы при умеренных скоростях робота
...
![Лабораторная работа 3](photo/LB3_MBR.gif)
